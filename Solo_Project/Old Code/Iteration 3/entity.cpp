...
void Entity::draw(sf::RenderTarget& t, sf::RenderStates s) const{
	...
	// more sophisicated cases of drawing
	// will be added when basic barebones draft
	// of shooting project is complete
	t.draw(eSprite);

	// Because the rendestate has not been properly initalized,
	// you have to settle with just the render target (esprite) for now
